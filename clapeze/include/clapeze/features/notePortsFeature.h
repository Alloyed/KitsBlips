...
class MidiNotePortsFeature : public BaseFeature {
   public:
    MidiNotePortsFeature(uint32_t numInputs, uint32_t numOutputs) : NUM_INPUTS(numInputs), NUM_OUTPUTS(numOutputs) {}
    static constexpr auto NAME = CLAP_EXT_NOTE_PORTS;
    const char* Name() const override { return NAME; }
   ...
   private:
    ...
    uint32_t NUM_OUTPUTS;
    static uint32_t _count([[maybe_unused]] const clap_plugin_t* plugin, bool isInput) {
        MidiNotePortsFeature& self = MidiNotePortsFeature::GetFromPluginObject<MidiNotePortsFeature>(plugin);
        if (isInput) {
            return self.NUM_INPUTS;
        } else {
            return self.NUM_OUTPUTS;
