...
inline WindowingApi toApiEnum(const char* api) 
{
    ...
    if(api == CLAP_WINDOW_API_X11) { return WindowingApi::X11; }
    if(api == CLAP_WINDOW_API_WAYLAND) { return WindowingApi::Wayland; }
    if(api == CLAP_WINDOW_API_WIN32) { return WindowingApi::Win32; }
    if(api == CLAP_WINDOW_API_COCOA) { return WindowingApi::Cocoa; }
    // should never happen
...
class GuiExt: public BaseExt {
   ...
   private:
    ...
    static bool _get_preferred_api(const clap_plugin_t *plugin, const char **apiString, bool *isFloating)  {
        ...
        switch(api) {
            case WindowingApi::X11: { *apiString = CLAP_WINDOW_API_X11; break; }
            case WindowingApi::Wayland: { *apiString = CLAP_WINDOW_API_WAYLAND; break; }
            case WindowingApi::Win32: { *apiString = CLAP_WINDOW_API_WIN32; break; }
            case WindowingApi::Cocoa: { *apiString = CLAP_WINDOW_API_COCOA; break; }
            case WindowingApi::None: { return false; }
        }
        return true;
    }
