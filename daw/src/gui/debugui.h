...
template <typename T>
inline void DebugEnumParam(clapeze::params::DynamicParametersFeature& feature, clap_id index) {
    auto& handle = feature.GetMainHandle();
    double raw = handle.GetRawValue(index);

    clapeze::BaseParam* base = feature.GetBaseParam(index);
    if (auto* maybeEnum = dynamic_cast<clapeze::EnumParam<T>*>(base)) {
...
inline void DebugParam(clapeze::params::DynamicParametersFeature& feature, clap_id index) {
    ...
    if (auto* maybeInteger = dynamic_cast<clapeze::IntegerParam*>(base)) {
    ...
    } else if (auto* maybeNumeric = dynamic_cast<clapeze::NumericParam*>(base)) {
