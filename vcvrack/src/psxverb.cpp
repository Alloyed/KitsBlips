...
struct PSXVerb : Module {
    ...
    static constexpr size_t psxBufferSize = 65536;  // PSX::GetBufferDesiredSizeFloats(PSX::kOriginalSampleRate);
    float psxBuffer[psxBufferSize];
    PSX::Reverb psx{PSX::kOriginalSampleRate, psxBuffer, psxBufferSize};
    kitdsp::Resampler<float_2> psxSampler{PSX::kOriginalSampleRate, PSX::kOriginalSampleRate};
...
struct PSXVerbWidget : ModuleWidget {
    PSXVerbWidget(PSXVerb* module) {
        ...
        addChild(
            createLightCentered<MediumLight<RedLight>>(mm2px(Vec(x2 - 2, y - 2)), module, PSXVerb::PRESET_1_LIGHT));
        addChild(
            createLightCentered<MediumLight<RedLight>>(mm2px(Vec(x2 + 2, y - 2)), module, PSXVerb::PRESET_2_LIGHT));
        addChild(
            createLightCentered<MediumLight<RedLight>>(mm2px(Vec(x2 - 2, y + 2)), module, PSXVerb::PRESET_3_LIGHT));
        addChild(
            createLightCentered<MediumLight<RedLight>>(mm2px(Vec(x2 + 2, y + 2)), module, PSXVerb::PRESET_4_LIGHT));
        y += ynext;
