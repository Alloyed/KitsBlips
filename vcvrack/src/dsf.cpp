...
static constexpr float cModulatorRatios[] = {0.0f, 0.25f, 0.5f, 1.f,  2.f,
                                             3.f,  4.f,   5.f,  10.0f};
...
struct Dsf : Module {
    ...
    Dsf() {
        ...
        configSwitch(MOD_MODE_PARAM, 0.f, 1.f, 0.f, "Modulator Pitch tracking",
                     {"Ratio", "Fixed"});
    ...
    void process(const ProcessArgs& args) override {
        if (!outputs[OUT_MAIN_OUTPUT].isConnected() &&
            !outputs[OUT_AUX_OUTPUT].isConnected()) {
            // nothing to compute!
            return;
        }
        ...
        for (int c = 0; c < channels; c += 4) {
            ...
            {
                float4 pitch =
                    octave + inputs[PITCH_INPUT].getPolyVoltageSimd<float4>(c) +
                    (params[FINE_PARAM].getValue() / 12.0f);
                float4 fm = params[FM_PARAM].getValue() *
                            inputs[FM_INPUT].getPolyVoltageSimd<float4>(c);
            ...
            if (useRatioModulator) {
                ...
                float4 ratioCoarse = simd::clamp(
                    params[MOD_COARSE_PARAM].getValue() + 2.0f +
                        inputs[MOD_PITCH_INPUT].getPolyVoltageSimd<float4>(c),
                    0.0f, 6.0f);
                ...
                if (ratioFine < 0.0f) {
                    ...
                    ratio =
                        lerpf<float4>(ratio0, ratio1, simd::fabs(ratioFine));
                } else {
                ...
                float4 modfm =
                    params[MOD_FM_PARAM].getValue() *
                    inputs[MOD_FM_INPUT].getPolyVoltageSimd<float4>(c) *
                    dsp::FREQ_C4;
                ...
                float4 pitch =
                    modOctave + modFine +
                    inputs[MOD_PITCH_INPUT].getPolyVoltageSimd<float4>(c);
                float4 fm = params[MOD_FM_PARAM].getValue() *
                            inputs[MOD_FM_INPUT].getPolyVoltageSimd<float4>(c);
            ...
            float4 level = simd::clamp(
                params[LEVEL_PARAM].getValue() +
                    (inputs[LEVEL_INPUT].getPolyVoltageSimd<float4>(c) * 0.1f *
                     params[LEVEL_CV_PARAM].getValue()),
                0.0f, 1.0f);
            float4 falloff =
                level *
                0.9f;  // numerical instability/volume issues closer to 1.0f
...
struct DsfWidget : ModuleWidget {
    DsfWidget(Dsf* module) {
        ...
        addParam(createParamCentered<NKK2>(mm2px(Vec(55.239, 29.905)), module,
                                           Dsf::MOD_MODE_PARAM));
        addParam(createParamCentered<Davies1900hBlackKnob>(
            mm2px(Vec(35.414, 31.587)), module, Dsf::MOD_COARSE_PARAM));
        addParam(createParamCentered<Davies1900hBlackKnob>(
            mm2px(Vec(13.026, 31.956)), module, Dsf::COARSE_PARAM));
        addParam(createParamCentered<Davies1900hBlackKnob>(
            mm2px(Vec(35.613, 51.719)), module, Dsf::MOD_FINE_PARAM));
        addParam(createParamCentered<Davies1900hBlackKnob>(
            mm2px(Vec(55.873, 51.719)), module, Dsf::LEVEL_PARAM));
        addParam(createParamCentered<Davies1900hBlackKnob>(
            mm2px(Vec(11.775, 52.419)), module, Dsf::FINE_PARAM));
        addParam(createParamCentered<Trimpot>(mm2px(Vec(11.777, 72.881)),
                                              module, Dsf::FM_PARAM));
        addParam(createParamCentered<Trimpot>(mm2px(Vec(35.667, 72.881)),
                                              module, Dsf::MOD_FM_PARAM));
        addParam(createParamCentered<Trimpot>(mm2px(Vec(56.075, 72.881)),
                                              module, Dsf::LEVEL_CV_PARAM));

        addInput(createInputCentered<PJ301MPort>(mm2px(Vec(11.773, 90.193)),
                                                 module, Dsf::FM_INPUT));
        addInput(createInputCentered<PJ301MPort>(mm2px(Vec(35.663, 90.193)),
                                                 module, Dsf::MOD_FM_INPUT));
        addInput(createInputCentered<PJ301MPort>(mm2px(Vec(55.92, 90.193)),
                                                 module, Dsf::LEVEL_INPUT));
        addInput(createInputCentered<PJ301MPort>(mm2px(Vec(18.013, 112.103)),
                                                 module, Dsf::PITCH_INPUT));
        addInput(createInputCentered<PJ301MPort>(mm2px(Vec(30.258, 112.103)),
                                                 module, Dsf::MOD_PITCH_INPUT));

        addOutput(createOutputCentered<PJ301MPort>(
            mm2px(Vec(45.424, 112.103)), module, Dsf::OUT_MAIN_OUTPUT));
        addOutput(createOutputCentered<PJ301MPort>(
            mm2px(Vec(57.366, 112.103)), module, Dsf::OUT_AUX_OUTPUT));
    }
};
