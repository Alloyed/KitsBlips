...
namespace kitdsp {
    GrainPlayer::GrainPlayer(etl::span<float> buffer, float sampleRate)
        :mDelayLine(buffer),
        mSampleRate(sampleRate)
    {}
    void GrainPlayer::Reset() {
    }

    void GrainPlayer::SetParams(float transpose, size_t grainSize) {
    }

    float GrainPlayer::Process(float in) {
    ...
    size_t GrainPlayer::GetMaxGrainSize() const {
    }

    size_t GrainPlayer::GetEffectiveDelay() const {
    }
}
