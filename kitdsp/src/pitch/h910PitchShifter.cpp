...
H910PitchShifter::H910PitchShifter(etl::span<float> buffer, float sampleRate)
    : mDelayLine(buffer), mSampleRate(sampleRate) {
...
void H910PitchShifter::Reset() {}

void H910PitchShifter::SetParams(float pitchRatio, float grainSizeMs, float baseDelayMs, float feedback) {
    mBaseDelaySamples = (baseDelayMs * mSampleRate / 1000.0f) + 1;
...
float H910PitchShifter::Process(float in) {
...
size_t H910PitchShifter::GetMaxGrainSize() const {
    return mDelayLine.Size();
}

size_t H910PitchShifter::GetEffectiveDelay() const {
    return 0;  // idk man
}
}  // namespace kitdsp
