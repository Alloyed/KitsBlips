...
struct Config {
    ...
    // kMaxEchoMs. corresponds to the EDL register
    ...
    float echoFeedback;
    // [-1, 1]: moves the read head up/down up to 16 ms. can be used for
    // chorus-y effects. (extension, not available on original hardware)
    float echoDelayMod;
    // [0, kNumFilterSettings]: select from a number of FIR filter presets
    // harvested from actual games. corresponds to FIRx register
    uint8_t filterSetting;
    // [0, 1]: FIR mix setting. 0 is no filter, 1 is full filter (as existed on
    // the hardware)
    float filterMix;
...
class Model {
   ...
   private:
    ...
    // any coeffecients are allowed, but historically accurate coeffecients can
    // be found here: https://sneslab.net/wiki/FIR_Filter#Examples
