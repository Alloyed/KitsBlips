...
TEST(stft, detectsSineWaves) {
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        float freq = kitdsp::max(0.0001f, static_cast<float>(i) * SR / static_cast<float>(N * 2));
...
TEST(stft, detectsSawWaves) {
    float SR = 44100.0f;
    kitdsp::naive::PulseOscillator ph;
    std::vector<float> buf;
    buf.resize(kitdsp::STFT<1024>::GetDesiredBufferSize());
    kitdsp::STFT<1024> fft(buf);
    size_t len = static_cast<size_t>(SR * 2.0f);

    ph.SetFrequency(220.0f, SR);
    for (size_t i = 0; i < len; ++i) {
        fft.Process(ph.Process());
    }
    kitdsp::test::CsvFile<float, float, float> f;
    f.columns = {"frequency", "magnitude", "phase"};
    size_t N = fft.mOut.size() / 2;
